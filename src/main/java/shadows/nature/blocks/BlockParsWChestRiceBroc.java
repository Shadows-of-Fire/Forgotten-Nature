...
public class BlockParsWChestRiceBroc extends BlockCrops
{
    ...
    public BlockParsWChestRiceBroc()
    {
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister)
    {
        ...
        {
        	if(i < 4)
            	this.cropIcons[i] = par1IconRegister.registerIcon("ForgottenNature:Parsley_" + i);
        	else if(i > 3 && i < 8)
        		this.cropIcons[i] = par1IconRegister.registerIcon("ForgottenNature:Broccoli_" + (i - 4));
        	else if(i > 7 && i < 12)
        		this.cropIcons[i] = par1IconRegister.registerIcon("ForgottenNature:WaterChestnut_" + (i - 8));
        	else if(i > 11)
        		this.cropIcons[i] = par1IconRegister.registerIcon("ForgottenNature:Rice_" + (i - 12));
        	}
    }
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int met, float par6, int par7)
    {
        ...
        if (met == 3)
        {
        	dropBlockAsItem(world, i, j, k, new ItemStack(food1,1,7));//parsley
        }
        else if (met == 7)
        {
        	dropBlockAsItem(world, i, j, k, new ItemStack(food1,1,15));//broccoli
        }
        else if (met == 11)
        {
        	dropBlockAsItem(world, i, j, k, new ItemStack(food1,1,9));//wChest
        }
        else if (met == 15)
        {
        	dropBlockAsItem(world, i, j, k, new ItemStack(food1,1,10));//rice
        }
        ...
        for (int n = 0; n < 5; n++)
        {
            ...
            {
            	dropBlockAsItem(world, i, j, k, new ItemStack(seed1,1,10));
            }
            else if ((met == 7) && (world.rand.nextInt(3) == 0))
            {
            	dropBlockAsItem(world, i, j, k, new ItemStack(seed1,1,9));
            }
            ...
            {
            	dropBlockAsItem(world, i, j, k, new ItemStack(seed1,1,3));
            }
        }
    }
