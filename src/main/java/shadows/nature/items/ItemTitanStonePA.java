...
public class ItemTitanStonePA extends ItemTool {
	...
	private static Set blocksEffectiveAgainst = Sets.newHashSet(new Block[] { FNBlocks.newPlanks1, FNBlocks.newLogs1,
			FNBlocks.newLogs2, FNBlocks.newLogs3, Blocks.planks, Blocks.bookshelf, Blocks.log, Blocks.log2,
			Blocks.chest, Blocks.wooden_slab, Blocks.double_wooden_slab, Blocks.pumpkin, Blocks.lit_pumpkin,
			FNBlocks.crystalWood, Blocks.grass, Blocks.dirt, Blocks.sand, Blocks.gravel, Blocks.snow, Blocks.snow_layer,
			Blocks.clay, Blocks.farmland, Blocks.soul_sand, Blocks.mycelium, Blocks.cobblestone,
					...
					for (int i = -1; i <= 1; i++) {
						for (int j = 0; j <= 2; j++) {
							for (int k = -1; k <= 1; k++) {
								...
								if (blk != Blocks.bedrock && blk != Blocks.water && blk != Blocks.flowing_water
										&& blk != Blocks.lava && blk != Blocks.flowing_lava) {
