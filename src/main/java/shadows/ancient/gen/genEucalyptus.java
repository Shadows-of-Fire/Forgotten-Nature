...
public class genEucalyptus extends WorldGenerator {
	Block leaf;
	int leafmeta;
	Block log;
	int logmeta;

	public genEucalyptus(Block l, int lm, Block i, int k) {
		leaf = l;
		leafmeta = lm;
		log = i;
		logmeta = k;
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for (int k1 = 1; k1 <= l; k1++) {
			if (world.isAirBlock(i, j + k1, k) || k1 == 0)
				world.setBlock(i, j + k1, k, log, logmeta, 3);
		}
